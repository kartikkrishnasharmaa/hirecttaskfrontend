  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        setTask(data.task);
        setFormData({ description: data.task.description, title:data.task.title });
      });
    }
        ...
        <form className='m-auto my-16 max-w-[1000px] bg-white p-8 border-2 shadow-md rounded-md'>
          {loading ? (
            ...
            <>
                ...
                <Textarea type="title" name="title" id="title" value={formData.title} placeholder="Task Title.." onChange={handleChange} />
                {fieldError("title")}
              </div>
              <div className="mb-4">
              ...
              <button className='bg-blue-600 text-white px-4 py-2 font-medium hover:bg-blue-600-dark' onClick={handleSubmit}>{mode === "add" ? "Add task" : "Update Task"}</button>
